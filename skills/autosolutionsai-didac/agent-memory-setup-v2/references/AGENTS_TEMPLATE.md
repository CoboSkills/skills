...
### 🔍 Semantic Search (Gemini Embeddings 2)

Memory search uses **Google Gemini Embeddings 2** (`gemini-embedding-2-preview`) via the built-in `memory-core` plugin. This means:
- `memory_search` finds memories by **semantic meaning**, not just keywords
- Embeddings are computed in the cloud — no local model downloads needed
- Requires a **Gemini API key** (free tier available at https://aistudio.google.com/apikey)

The agent's `memory_search` tool automatically uses Gemini embeddings when configured.
