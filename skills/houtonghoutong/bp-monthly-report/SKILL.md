...
description: Use when drafting a monthly BP report from a fixed template, a fill-in specification, person or node metadata, and real BP or progress-report evidence. This skill enforces a staged workflow: normalize the template, map BP anchors, collect evidence, build section cards, then draft the report in a fixed section order instead of generating the whole report in one pass.
---
