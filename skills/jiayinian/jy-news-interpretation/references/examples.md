...
**工具调用**：
```bash
mcporter call jy-financedata-api.get24hHotNews query="过去 24 小时财经热点新闻" --output json
```
...
**工具调用**：
```bash
mcporter call jy-financedata-tool.interpretNews query="美联储加息新闻解读" --output json
```
...
**工具调用**：
```bash
mcporter call jy-financedata-api.analyzeIndustryNews query="新能源行业政策新闻影响" --output json
```
...
**工具调用**：
```bash
mcporter call jy-financedata-api.queryStockAnnouncement query="600519.SH 最新公告" --output json
```
...
**工具调用**：
```bash
mcporter call jy-financedata-tool.findRelatedStocks query="中东局势 原油 A 股关联" --output json
```
...
**Markdown 输出（默认）**
```bash
mcporter call jy-financedata-api.get24hHotNews query="今日财经热点" --output json
```

**HTML 输出**
```bash
mcporter call jy-financedata-api.get24hHotNews query="今日财经热点" --output html
```

**JSON 输出（程序化处理）**
```bash
mcporter call jy-financedata-api.get24hHotNews query="今日财经热点" --output json --raw
```
