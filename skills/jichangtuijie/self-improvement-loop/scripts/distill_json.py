...
def calc_trigger(notified, nc, cnt):
    try:
        n = int(notified)
        if n <= 0: return True  # null/-1/0 → trigger
...
if len(sys.argv) < 4:
    print("Usage: distill_json.py <pk_agg> <cat_agg> <threshold> [distill_self]")
    sys.exit(1)

pk_agg_path = sys.argv[1]
cat_agg_path = sys.argv[2]
threshold = int(sys.argv[3])
distill_self = sys.argv[4] if len(sys.argv) > 4 else ""
...
for line in pk_agg:
    ...
    if not e: continue
    n_val = e["notified"]
    trig = 1 if calc_trigger(n_val, e["nc"], e["cnt"]) else 0
    obj = {
        "name": e["pk"], "count": e["cnt"], "threshold_count": threshold,
...
result = {
    ...
    "meta": {
        "threshold": threshold,
        "files_scanned": ["LEARNINGS.md", "ERRORS.md", "FEATURE_REQUESTS.md", "DISTILL-SELF.md", "龙虾成长闭环设计文档.md", "HOOK.md", "handler.js"],
        ...
        "self_fixes": {"file": distill_self, "needs_human_count": 0, "needs_human_ids": []},
