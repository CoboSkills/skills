...
首次运行任何命令时，runtime 会自动生成钱包并存入 `~/.moltx/wallet.json`（权限 `0o600`，仅当前用户可读写）。

```bash
node runtime/dist/cli.js call get_wallet_info --json '{}'
```

返回地址后，向该地址转入 ETH（gas 费）和 USDC（赏金/deposit）即可开始使用。
...
用私钥完成 SIWE 签名，runtime 自动写入 `~/.moltx/auth.json`，后续调用自动使用：
